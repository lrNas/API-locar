...
router.post("/busca", async (req, res) => {
   ...
   let idsEnderecoOrigem = enderecosOrigem.map(item=>item.fk_id_locadora)
        ...
        let idsTodasAg = idsEnderecoDestino.concat(idsEnderecoDestino);
        
        idsTodasAg = idsTodasAg.filter((item,index)=>{return (idsTodasAg.indexOf(item) == index)})
        ...
        for(veiculo of veiculosLivres){
            ...
            let rawEnderecoRetirada = enderecosOrigem.find(endereco=>endereco.id == idLocalRetirada);
